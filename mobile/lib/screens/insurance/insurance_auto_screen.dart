...
/// Mirrors pages/insurance/auto/index.js: a 4-step live quote/pay/issue
/// wizard against AAS Assurances, separate from the flat insurance-plan
/// catalog in insurance_screen.dart. Step 0 collects vehicle details and
/// fetches metadata; step 1 shows live per-tier quotes; step 2 collects
/// issuance/party details and charges the wallet; step 3 shows the result -
/// including the refunded-failure case (AAS accepted payment then failed
/// to issue) which must be told apart from a generic error.
...
class _InsuranceAutoScreenState extends State<InsuranceAutoScreen> {
  ...
  bool get _canCompare {
    if (_genre.isEmpty || _dureeController.text.trim().isEmpty) return false;
    if (_isMoto) {
  ...
  Widget _buildVehicleStep(BuildContext context) {
    final metadata = _metadata;
    final supportedGenres = (metadata?.genres ?? const <AasGenre>[]).where((g) => g.supported).toList();
    return Column(
    ...
    return Column(
      ...
      children: [
        ...
        ...quotes.results.map((r) {
          final labels = r.garanties
              .map((code) => metadata?.garantieLabels[code])
              .whereType<String>()
              .join(', ');
          return Opacity(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (labels.isNotEmpty)
                    Text(labels, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                  if (r.available) ...[
